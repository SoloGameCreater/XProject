...
public class ComponentBinderAttribute : Attribute
{
    ...
    public ComponentBinderAttribute(string path, string parentName = "")
    {
        mPath = path;
        mParent = parentName;
    }
...
public class ComponentBinder
{
    public static void BindingComponent(object binder, Transform transform)
    {
        ...
        Type typeAttr = typeof(ComponentBinderAttribute);
        ...
        foreach (var item in fieldInfoDict)
        {
            ...
            if (objAttr.Length > 0)
            {
                attr = objAttr[0] as ComponentBinderAttribute;
            }
        ...
        foreach (var method in listMethodInfo)
        {
            ...
            var attribute = (ComponentBinderAttribute) objAttr[0];
            if (attribute == null)
                continue;
    ...
    private static Object FindMatchComponent(Transform transform, string path, string parentName, Type type,
        Dictionary<string, List<Transform>> componentDict)
    {
        ...
        {
            if (type == typeof(GameObject)) return tran.gameObject;
        ...
        if (!path.Contains("/"))
        {
            if (componentDict.TryGetValue(path, out var list))
            {
                if (string.IsNullOrEmpty(parentName))
                {
                    ...
                    {
                        if (path == list[0].name) return list[0].gameObject;
                    }
                ...
                else
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        {
                            if (type == typeof(GameObject)) return item.gameObject;
