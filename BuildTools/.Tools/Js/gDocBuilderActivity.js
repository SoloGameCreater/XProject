...
var Builder = require('./BuilderUtilsActivity');
...
Handlebars.registerHelper("isSingleTable", function(sheetName, options) {
...
var outputConfig = new Builder.OutputConfig(program.args[1], program.args[2], program.args[3], program.args[4], program.args[5], program.args[6], program.args[7]);
...
function readsheet(sheetName, sheetData, sheetIndex) {
    ...
    for (k in rows[0]) {
        ...
        if (!sheetName.startsWith('Timeline') || !val.startsWith('_')) {
    ...
    if (sheetName.startsWith('Timeline')) {
        properties.push('_Combine');
        valueTypes['_Combine'] = 'arraystring2';
        descriptions['_Combine'] = '_Combine';
    }
    
    var gssClass = new Builder.GssClass(sheetName, properties, valueTypes, descriptions, program.args[3], program.args[7]);
