...
public class AssetTreeView : TreeView
{
    ...
    public bool PNG_FILTER = true;
    public bool OTHER_FILTER = true;
    ...
    protected override void RowGUI(RowGUIArgs args)
    {
        ...
        {
            if (PNG_FILTER && (item.data.path.ToLower().EndsWith(".png") || item.data.path.ToLower().EndsWith(".jpg") || item.data.path.ToLower().EndsWith(".jpge")))
            {
            }
            else if (OTHER_FILTER)
            {
            }
            else
            {
            ...
            {
                if (!item.data.path.ToLower().Contains(NAME_FILTER.ToLower()))
                    continue;
            }
    ...
    void CellGUI(Rect cellRect, AssetViewItem item, MyColumns column, ref RowGUIArgs args)
    {
        ...
        switch (column)
        {
            ...
            case MyColumns.Path:
            {
                ...
                // if (!path.StartsWith("Assets/Export/Textures/Decoration/Brush/")) Debug.LogError(path);
                // if (!path.ToLower().Contains("default")) Debug.LogError(path);
            }
                break;
