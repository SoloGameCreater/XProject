...
public class ReferenceFinderWindow : EditorWindow
{
    ...
    //过滤
    private bool _pngFilter = true;
    private bool _otherFilter = true;
    ...
    public void DrawOptionBar()
    {
        ...
        //过滤
        GUILayout.Space(10);
        _otherFilter = GUILayout.Toggle(_otherFilter, "其他", _toolbarButtonGUIStyle);
        _pngFilter = GUILayout.Toggle(_pngFilter, "图片", _toolbarButtonGUIStyle);
        _nameFilter = GUILayout.TextField(_nameFilter);

        if (_assetTreeView != null)
        {
            _assetTreeView.PNG_FILTER = _pngFilter;
            _assetTreeView.OTHER_FILTER = _otherFilter;
            _assetTreeView.NAME_FILTER = _nameFilter;
        }
